...
def get_unseen_valid_df(data_dir):
    ...
    valid_age_unseen_control_df = controls_df[controls_df['file_name'].isin(all_files)]
    valid_age_unseen_patients_df = patients_df[patients_df['file_name'].isin(all_files)]
...
def create_control_tle_plot(control_images, tle_images):
    fig, ax = plt.subplots(1, 3,figsize=[14, 6])
    for axis in ax.ravel():
        axis.set_axis_off()

    left = cv2.rotate(control_images, cv2.ROTATE_90_COUNTERCLOCKWISE)
    right = cv2.rotate(tle_images, cv2.ROTATE_90_COUNTERCLOCKWISE)
    sub = right - left
    ...
    ax[1].title.set_text('TLE')
    ax[2].title.set_text('TLE-Control')
    cbar = fig.colorbar(im, ax=ax[2], location="right", shrink=0.6, fraction=0.1)
    cbar.ax.tick_params(labelsize=10)

    fig.patch.set_facecolor('#FFFFFF')
    plt.savefig(os.path.join(save_dir, "control_tle_2D.png"))
    plt.show()
...
def create_animation(generated_images_transition):
    ...
    def update(i):
        ...
        if i == 0 and cbar is None:
            ...
            ax[0].set_title('Control', fontsize=10)  # Set top title
            ax[1].set_title('TLE', fontsize=10)  # Set top title
            ax[2].set_title('TLE-Control', fontsize=10)  # Set top title
    ...
    plt.close()
    HTML(ani.to_jshtml())
    ani.save(os.path.join(save_dir, "control_tle_format_2D.gif"))
...
if __name__ == "__main__":
    ...
    valid_age_unseen_control_df, valid_age_unseen_patients_df = get_unseen_valid_df(data_dir)
    ...
    avg_control_latent = np.mean(control_latents, axis=0)
    avg_patient_latent = np.mean(patient_latents, axis=0)

    batch_size = 1
    const_input = [tf.ones((batch_size, 1))]
    noise = gan.get_noise(batch_size)

    temp_c = np.expand_dims(avg_control_latent, axis = 0)
    temp_p = np.expand_dims(avg_patient_latent, axis = 0)
    ...
    create_control_tle_plot(generated_images[0,0,:,:,0], generated_images[1,0,:,:,0])

    print("Creating animation. This may take some time.")
    transition_latents = np.linspace(avg_control_latent, avg_patient_latent, num=100)
